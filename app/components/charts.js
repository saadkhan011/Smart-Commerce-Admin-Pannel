...
const Chart = ({ graphData }) => {
  ...
  const { data: buyerData, isLoading } = useGetQuery({
    queryKey: ["Buyer"],
    url: `buyer?limit=${100000}`,
  });
  ...
  const [chartData, setChartData] = useState({
    ...
    options: {
      ...
      colors: ["#5a46cf"], // Color for the chart
      xaxis: {
  ...
  const fetchSalesData = async () => {
    ...
    // Add buyer only if selectedBuyer has a value
    if (selectedBuyer) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4">
        <div className="bg-white rounded-lg shadow-md h-80 md:col-span-2">
          <div className="flex justify-between items-center p-2">
            ...
            <div>
              <select
                className="border rounded p-1 px-3 bg-blue-50 border-blue-600 text-black"
                value={selectedMonth}
              ...
              <select
                className="border rounded p-1 px-3 bg-blue-50 border-blue-600 text-black ms-4"
                value={selectedBuyer}
                onChange={(e) => setSelectedBuyer(e.target.value)}
              >
                <option value="">All</option>
                {buyerData &&
                  Array.isArray(buyerData.Buyers) &&
                  buyerData.buyers.map((element, index) => (
        ...
        <div className="bg-white rounded-lg shadow-md h-80 flex flex-col items-center justify-between p-4">
          ...
          <div className="flex flex-row justify-around w-full">
            <p className="text-black">
              <span>
                <PiCircleNotchFill className="inline text-blue-500 text-sm" />
              </span>{" "}
              Completed
            </p>
            <p className="text-black">
