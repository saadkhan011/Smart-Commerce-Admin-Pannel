      ...
      <div className="flex flex-wrap gap-2">
        {data.subcategories.map((subcategory) => (
          <p
            ...
            type="primary"
            className="bg-red-100  text-red-500 px-5 py-1 rounded-full"
          >
      ...
      <span className="ml-2">
        {data?.subcategoryName ? (
          ...
          <Tooltip title="This product is not visible because the subcategory has been changed. To display this product, change the subcategory.">
            <span className="text-red-500">Subcategory Missing</span>
          </Tooltip>
        )}
      </span>
    ),
...
export const getRestaurantColumn = (menu) => [
  {
  ...
  },
  {
    ...
    key: "action",
    render: (text, record) => (
      <Dropdown className="bg-gray-100" overlay={menu(record)}>
        <Button>
          <VscKebabVertical />
        </Button>
      </Dropdown>
...
export const getRestaurnatMobileColumn = (menu)=>[
  {
    title: "Details",
    render: (record) => (
      <div className="flex flex-col p-4 bg-gray-100 rounded-md mb-4">
...
export const getOrderColumns = (menu)=> [
  ...
  },
  {
    title: "Date",
    render: (data) => {
      const formattedDate = data?.orderDate.slice(0, 10).split('-').reverse().join('-');
  ...
  },
  {
    ...
    render: (data) => (
      <div className="flex flex-col">
        <span className="ml-2">{data?.restaurantId?.name}</span>
        <span className="ml-2">{data?.restaurantId?.email}</span>
      </div>
    ),
    className: "hidden md:table-cell",
  },
