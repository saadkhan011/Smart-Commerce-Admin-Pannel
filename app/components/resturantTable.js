...
import { getRestaurantColumn, getRestaurnatMobileColumn } from "./column";
import {
  GoogleMap,
  Marker,
  LoadScript,
...
const RestaurantTable = () => {
  ...
  const [restaurantAllData, setRestaurantAllData] = useState();
  ...
  useEffect(() => {
    ...
    if (data) {
      setFilteredData(data.restaurants); // Set filtered data to full data initially
    }
    // fetchData();
  }, [data, dateRange]);
  ...
  const showModal = async () => {
    ...
    const response = await axios.get(`${API_BASE_URL}/restaurant?limit=1000`,{
      headers: {
        "x-access-token": token, // Add the x-access-token to the headers
      },
    });
    setRestaurantAllData(response?.data?.restaurants);
  ...
  const addFees = async () => {
    try {
      ...
      const { restaurant, percentage } = values;
      let url = "admin/assign-fee-to-restaurant";
      let queryKey = "Restaurant";
      updateMutation.mutate({
        data: { fees: percentage, restaurantId: restaurant },
        url,
        queryKey,
      });
  ...
  const columns = getRestaurantColumn(menu);
  const mobileColumns = getRestaurnatMobileColumn(menu);

  const handleStatusChange = (record, status) => {
    let url = `restaurant/change-status/${record?._id}`;
    console.log(record);
    updateMutation.mutate({
      data: { status },
      url,
  ...
  const typeOfMember = async (e, con) => {
    ...
    if (value === "All") {
      setFilteredData(data.restaurants); // Show all data if "All" is selected
      return;
    }

    try {
      // Fetch the filtered data based on the selected member type
      const data = await getFilteredMembers(value, "restaurant", token);
      console.log(data);
  ...
  const fetchData = async (startDate, endDate) => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/restaurant/filter-by-date/${startDate}/${endDate}`,{
          headers: {
            "x-access-token": token, // Add the x-access-token to the headers
          },
        },
  ...
  return (
    <div className="container mx-auto px-4 py-4 order-table-dashoboard-restaurent">
      <ToastContainer />
      <div className="flex flex-col md:flex-row justify-between mb-4">
        <div className="flex gap-2">
          <Dropdown overlay={menuStatus}>
            <Button className="bg-[#D4041C14] text-gray-500 font-normal border-none w-1/2 md:w-[128px] h-[40px]">
              {memberType === "Pending" ||
              memberType === "Approve" ||
              memberType === "Reject"
                ? memberType
        ...
        <Button
            className="bg-[#D4041C14] text-gray-500 border-none py-5 font-medium"
            onClick={showModal}
          >
            {" "}
            Add Percentage
          ...
          />
          <Button
            className="bg-[#E62F39] text-white font-medium border-none py-5 w-full md:w-auto"
            onClick={showDrawer}
          >
      ...
      <Drawer
        title={
          ...
          >
            <span>{editingRecord ? "Edit Restaurant" : "Add Restaurant"}</span>
            <CloseOutlined
      ...
      >
        ...
        >
          ...
          <p className="text-black font-semibold text-2xl mb-3">
            {editingRecord ? "Edit Restaurant" : "Add Restaurant"}
          </p>
          <Form.Item
            name="name"
          ...
          <Form.Item>
            <div className="flex justify-between gap-5">
              ...
              <button
                className="bg-[#E62F39] text-white border-none py-2 w-1/2 rounded-md"
                onClick={handleSave}
              >
                Save
              </button>
      ...
      >
        <Form layout="vertical " className="p-2" form={feeForm}>
          <Form.Item name="restaurant" label="Select Restaurant">
            <Select
              placeholder="Please select a restaurant"
              className="bg-[#EBEBEB3D]"
            >
              {restaurantAllData &&
                restaurantAllData.map((restaurant) => (
                  <Option key={restaurant.key} value={restaurant._id}>
                    {restaurant.name}
                  </Option>
                ))}
            </Select>
          ...
          <button
            className="bg-[#E62F39] text-white border-none py-3 w-full  rounded-md mx-auto"
            onClick={addFees}
          >
            {" "}
            save
...
export default withAuth(RestaurantTable);
