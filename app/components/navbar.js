...
const Navbar = () => {
  ...
  useEffect(() => {
    settoken(localStorage.getItem("meatmetokenAdmin"));
    const storedUser = localStorage.getItem("meatmeAdmin");
    if (storedUser) {
  ...
  const menuItems = [
    ...
    { name: "Restaurants", icon: FaUtensils, route: "/resturants" },
    { name: "Suppliers", icon: FaTruck, route: "/suppliers" },
    { name: "Products", icon: FaBox, route: "/products" },
    { name: "Categories", icon: FaTruck, route: "/category" },
    { name: "Orders", icon: FaShoppingCart, route: "/order" },
    { name: "Settings", icon: FaCog, route: "/settings" },
    { name: "Listings", icon: FaLine, route: "/listings" },
    // { name: "Fees", icon: FaPercent, route: "/fees" },
  ];
  ...
  const logoutButton = () => {
    localStorage.removeItem("meatmeAdmin");
    localStorage.removeItem("meatmetokenAdmin");
    router.push("/login");
  };
              ...
              <div className="hidden sm:ml-6 sm:flex sm:items-center">
                ...
                <div className="ml-3 relative">
                  <div className="flex items-center">
                    ...
                    <button
                      type="button"
                      className="bg-white flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                      id="user-menu-button"
                      aria-expanded="false"
                      aria-haspopup="true"
                      onClick={() => setMenuOpen(!menuOpen)}
                    >
                      <FaAngleDown className="h-6 w-6 text-red-500" size={25} />
                    </button>
                  </div>
      {/* Dropdown Menu */}
      {menuOpen && (
                ...
                {menuItems.map((item) => (
                  <li
                    ...
                    className={`flex items-center py-2 px-3 rounded-md cursor-pointer ${
                      active === item.name
                        ? "bg-[#D4041C]"
                        : "hover:bg-[#D4041C] hover:text-white"
                    }`}
                  ...
                  >
                    ...
                    >
                      ...
                      <span>{item.name}</span>
                                ...
                                <li
                  className="flex items-center py-2 px-3 rounded-md cursor-pointer hover:bg-[#D4041C] hover:text-white"
                  onClick={logoutButton}
                >
