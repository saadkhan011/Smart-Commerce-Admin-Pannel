...
const OrderChart = () => {
  ...
  useEffect(()=>{
    if(data){
      setSelectedDropdownData(data.supplier ? data.supplier : data.buyers);
    }
  })
  ...
  const fetchSalesData = async () => {
    ...
    if (selectedDropdownValue && type === "buyer") {
      url += `&buyerId=${selectedDropdownValue}`;
    }
          ...
          <div className="md:w-1/2 w-full mt-2 ">
            <Button
              className={`${
                selected === "Month"
                  ...
                  : "bg-white hover:bg-[#6C53FD] text-[#5a46cf] border border-[#5a46cf]"
            ...
            <Button
              className={`${
                selected === "Quarter"
                  ? "bg-[#5a46cf] text-white"
                  : "bg-white text-[#6C53FD] border border-[#5a46cf]"
