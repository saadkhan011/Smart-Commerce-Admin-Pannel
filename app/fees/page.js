...
const Page = () => {
  ...
  // Handle changes in fees for each Buyer
  const handleFeeChange = (value, buyerId) => {
  ...
  const handleSupplierSelect = (supplier) => {
    ...
    setSelectedSuppliers(supplier.buyers || []); // Use empty array if no buyer
        ...
        <div className="bg-white rounded-lg shadow-md p-10">
          ...
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: "Please enter fees for at least one buyer",
              },
            ]}
          >
            <div className="flex flex-col">
              {selectedSupplier?.buyers && selectedSuppliers.map(({ buyer, fees }) => (
                <div
                  key={buyer?._id}
                  className="flex items-start"
                >
                  <div className="w-1/4">
                  <p className="pt-2">{buyer?.name}</p>
                  </div>
                  <div>
                  <Input
                    ...
                    onChange={(e) =>
                      handleFeeChange(e.target.value, buyer._id)
                    }
                  />
                  <span> &nbsp;% </span>
                  </div>
              ...
              {selectedSuppliers.length === 0 && <span>No buyers available for this supplier.</span>}
            </div>
          </Form.Item>
        </div>
        <Form.Item>
